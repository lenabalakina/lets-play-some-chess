...
export async function setDrawOffer(db: DB, gameId: string, offeredBy: string): Promise<boolean> {
  const { data, error } = await db
    .from('games')
    .update({ draw_offered_by: offeredBy })
    .eq('id', gameId)
    .eq('status', 'active')
...
export async function completeAcceptedDraw(
  db: DB,
  gameId: string,
  offeredBy: string,
): Promise<boolean> {
