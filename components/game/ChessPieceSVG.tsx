...
import { PIECE_GLOW, PIECE_PALETTE } from '@/features/chess/types/chess.types'
import { PieceSvgShell, type PieceProps } from './pieceSvgShell'

export function PawnSVG(props: PieceProps) {
  const { fill, stroke, size = 45, glowColor, glowId } = props
  return (
    <PieceSvgShell size={size} glowColor={glowColor} glowId={glowId}>
      <path
      ...
      <path d="M14.5 30c0-3.87 3.13-7 8-7s8 3.13 8 7" fill={fill} stroke={stroke} strokeWidth="1.5"/>
    </PieceSvgShell>
  )
}

export function RookSVG({ fill, stroke, size = 45, glowColor, glowId }: PieceProps) {
  return (
    <PieceSvgShell size={size} glowColor={glowColor} glowId={glowId}>
      <path
...
export function KnightSVG({ fill, stroke, size = 45, glowColor, glowId }: PieceProps) {
  return (
    <PieceSvgShell size={size} glowColor={glowColor} glowId={glowId}>
      <path
...
export function BishopSVG({ fill, stroke, size = 45, glowColor, glowId }: PieceProps) {
  return (
    <PieceSvgShell size={size} glowColor={glowColor} glowId={glowId}>
      <path
...
export function QueenSVG({ fill, stroke, size = 45, glowColor, glowId }: PieceProps) {
  return (
    <PieceSvgShell size={size} glowColor={glowColor} glowId={glowId}>
...
export function KingSVG({ fill, stroke, size = 45, glowColor, glowId }: PieceProps) {
  return (
    <PieceSvgShell size={size} glowColor={glowColor} glowId={glowId}>
...
export function ChessPieceSVG({ type, isWhite, isSelected, size = 45 }: ChessPieceSVGProps) {
  ...
  const glowColor = isWhite ? PIECE_GLOW.white : PIECE_GLOW.black
  const glowId = `${isWhite ? 'w' : 'b'}-${uid}`
